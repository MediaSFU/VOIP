...
export const useOutgoingCallRoomManager = ({
  ...
  // Store outgoing room data using cross-platform storage
  const getStoredOutgoingRoom = useCallback(async () => {
    try {
      const stored = await storage.getItem('mediasfu_outgoing_room');
      if (stored) {
        ...
        if (
          ...
          await storage.removeItem('mediasfu_outgoing_room');
        }
      }
    } catch (error) {
  ...
  const setStoredOutgoingRoom = useCallback(async (room: OutgoingCallRoom | null) => {
    try {
      if (room) {
        await storage.setItem('mediasfu_outgoing_room', JSON.stringify(room));
